...
def _folder_tag(path_str: str) -> str:
    # Prefer model name if present
    lowered = path_str.lower()
    if "biobert" in lowered:
        return "BioBERT"
    elif "bioclinicalbert" in lowered:
        return "BioClinicalBERT"
    # Otherwise, use last folder name
    tag = os.path.basename(os.path.normpath(path_str))
    return tag if tag else path_str.replace(os.sep, "-").replace(" ", "_")

def build_knn_patient_graph(input_folder, output_folder, k=5, label_csv=None):
    os.makedirs(output_folder, exist_ok=True)

    # Build a descriptive suffix for output files
    input_tag = _folder_tag(input_folder)
    out_suffix = f"{input_tag}_k{k}"
    ...
    if label_csv:
        ...
        labels = torch.tensor([label_to_int[label] for label in label_list], dtype=torch.long)
    ...
    with open(os.path.join(output_folder, ids_fname), "w") as f:
        for pid in patient_ids:
            f.write(pid + "\n")

    print(f"\nSaved kNN graph with {num_patients} nodes and {edge_index.size(1)} edges to {os.path.join(output_folder, graph_fname)}")
