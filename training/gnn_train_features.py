...
def _run_slug(args) -> str:
    ...
    clin_tag  = Path(args.clinical_csv).name  # using clinical CSV basename in lieu of text_dir
...
def _interpolate_mean_std(fprs_tprs, num_points=200):
    """
    For a list of (fpr, tpr) pairs (across folds) compute mean±std ROC.
    Returns mean_fpr, mean_tpr, std_tpr, mean_auc, std_auc or None.
    """
    if not fprs_tprs:
        return None
...
def main(args):
    ...
    # Raw JSON (fold metrics only; ROC data is saved in NPZ next)
    raw_payload = {
        "fold_metrics": [
