...
def _mlp_layers_for_modality(modality: str) -> int:
    # Match your layer-counting table: Clinical=2, Mel=3, Clinical+Mel=4
...
def load_data_from_patient_csv(label_csv, clinical_csv, mel_dir, classification):
    ...
    clinical_feature_names = list(X.columns)
    ...
    return clinical_tensor, mel_tensor, labels, clinical_feature_names, mel_feature_names
...
def _ig_sample_mlp(model, x_all, idx, target_mask, steps=64, device="cpu"):
    ...
    Only features selected by target_mask are varied along the path; the others are held at their actual values.
    Baseline = mean feature vector over the dataset.
    """
    model.eval()
    ...
    # pick the predicted class of the actual input
    with torch.no_grad():
        logits_all = model(x_all)
        target_class = int(logits_all[idx].argmax().item())

    target_mask = target_mask.to(device)
    delta = (x_actual - baseline) * target_mask  # vary only target features

    acc_grads = torch.zeros_like(x_actual)
    for alpha in torch.linspace(0.0, 1.0, steps, device=device):
        z = x_actual.clone()
        # interpolate along selected features; others fixed at actual values
    ...
    ig = delta * (acc_grads / steps)  # (D,)
    # return only the selected features (others zero)
    return (ig * target_mask).detach().cpu()

def compute_global_feature_importance_mlp(model, x_all, feature_names, branch, clin_dim, steps=64, sample_count=0, device="cpu"):
    """
    branch: 'clinical' | 'mel' | 'both'
    clin_dim: number of clinical features (to split the concatenated vector when needed)
    """
    D = x_all.shape[1]
    if branch == "clinical":
        mask = torch.zeros(D)
        mask[:clin_dim] = 1.0
        names = feature_names[:clin_dim]
    elif branch == "mel":
        mask = torch.zeros(D)
        mask[clin_dim:] = 1.0
        names = feature_names[clin_dim:]
        ...
        raise ValueError("branch must be one of {'clinical','mel','both'}")
...
# -------------------- Main --------------------
def main(args):
    # Graph (needed only for GNN baselines; harmless for MLP)
    ...
    # Data
    clinical_embeddings, mel_embeddings, labels, clinical_feature_names, mel_feature_names = load_data_from_patient_csv(
    ...
    trained_model = None  # keep last model for explainability
    ...
    if args.explain:
        if not getattr(trained_model, "is_mlp", False):
            print("[WARN] --explain was set, but the trained model is not MLP. Skipping IG.")
        else:
            # Build the feature space used by the MLP for the chosen modality
            if args.modality == "clinical":
                x_all = graph_data.clinical
                feature_names = clinical_feature_names
                clin_dim = len(clinical_feature_names)
            elif args.modality == "mel":
                x_all = graph_data.mel
                feature_names = mel_feature_names
                clin_dim = 0  # no clinical part
            elif args.modality == "both":
                x_all = torch.cat([graph_data.clinical, graph_data.mel], dim=1)
                feature_names = clinical_feature_names + mel_feature_names
                clin_dim = len(clinical_feature_names)
            else:
                raise ValueError("Invalid modality")

            # Validate requested branch
            if args.explain_branch == "both" and args.modality != "both":
                print("[WARN] --explain_branch both requested but modality is not 'both'; defaulting to active features.")
                branch = "clinical" if args.modality == "clinical" else "mel"
            else:
            ...
            df_imp = compute_global_feature_importance_mlp(
                ...
                clin_dim=clin_dim, steps=args.ig_steps,
...
if __name__ == '__main__':
    ...
    # Explainability options (MLP)
    parser.add_argument('--explain', action='store_true', help="Compute feature importances (IG) and save barplot for MLP")
    parser.add_argument('--explain_branch', type=str, choices=['clinical','mel','both'], default='clinical',
    ...
    parser.add_argument('--explain_samples', type=int, default=0, help="Number of samples to use for IG (0=all)")
