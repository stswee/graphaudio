...
class MLP(nn.Module):
    """
    MLP with a specified number of HIDDEN layers (depth excludes output layer),
    matching the user's comparison-by-layer-count requirement.
    """
    def __init__(self, in_dim: int, out_dim: int, n_hidden_layers: int, hidden_dim: int = 512, dropout: float = 0.0):
        super().__init__()
        self.is_mlp = True  # used to branch forward/eval logic
        layers = []
        prev = in_dim

        # Build exactly n_hidden_layers fully-connected layers
        ...
        # Output layer (not counted toward the "learnable layers" for parity)
...
def _mlp_layers_for_modality(modality: str) -> int:
    """
    Match the MLP hidden-layer count to your 'Total Learnable Layers' convention.
    """
    if modality == 'text':
        return 2
    elif modality == 'mel':
...
def get_model(architecture, modality, input_dim_text, input_dim_mel, num_classes,
    ...
    if architecture == 'GCN':
        ...
        if modality == 'text':
            in_dim = input_dim_text
        elif modality == 'mel':
            in_dim = input_dim_mel
        elif modality == 'both':
            in_dim = input_dim_text + input_dim_mel
        else:
...
def evaluate(model, data, val_idx, labels, modality, device, classification):
    ...
    with torch.no_grad():
        if getattr(model, "is_mlp", False):
            # Build input vector depending on modality
            if modality == 'both':
                x = torch.cat([data.text, data.mel], dim=1).to(device)
            elif modality == 'text':
...
def _ig_sample_mlp(model, x_all, idx, target_mask, steps=64, device="cpu"):
    """
    Integrated Gradients for a single sample of an MLP.
    Baseline = mean feature vector. Only features where target_mask==1 are varied.
    """
    model.eval()
    x_all = x_all.to(device)
    baseline = x_all.mean(dim=0)  # (D,)
    x_actual = x_all[idx].detach()

    # class to attribute = predicted class
    with torch.no_grad():
        logits_all = model(x_all)
        target_class = int(logits_all[idx].argmax().item())

    target_mask = target_mask.to(device)
    delta = (x_actual - baseline) * target_mask  # vary only selected features

    acc_grads = torch.zeros_like(x_actual)
    for alpha in torch.linspace(0.0, 1.0, steps, device=device):
        z = x_actual.clone()
        # interpolate along selected features; others frozen at actual value
        z = z - target_mask * (z - (baseline + alpha * delta))
        z = z.requires_grad_(True)
        out = model(z.unsqueeze(0))[0, target_class]
        grads = torch.autograd.grad(out, z, retain_graph=False, create_graph=False)[0]
        acc_grads += grads

    ig = delta * (acc_grads / steps)
    return (ig * target_mask).detach().cpu()

def compute_global_feature_importance_mlp(model, x_all, feature_names, branch, text_dim, steps=64, sample_count=0, device="cpu"):
    """
    branch: 'text' | 'mel' | 'both'
    text_dim: number of text features (to split concatenated vector when needed)
    """
    D = x_all.shape[1]
    if branch == "text":
        mask = torch.zeros(D)
        mask[:text_dim] = 1.0
        names = feature_names[:text_dim]
    elif branch == "mel":
        mask = torch.zeros(D)
        mask[text_dim:] = 1.0
        names = feature_names[text_dim:]
        ...
        raise ValueError("branch must be one of {'text','mel','both'}")
    ...
    if (sample_count is None) or (sample_count <= 0) or (sample_count >= N):
        ...
        idxs = rng.choice(N, size=sample_count, replace=False).tolist()

    agg = torch.zeros(D)
    for i in tqdm(idxs, desc=f"Explaining samples (IG-MLP, branch={branch})"):
        ig = _ig_sample_mlp(model, x_all, i, target_mask=mask, steps=steps, device=device)
        agg += ig.abs()

    active = mask > 0
    values = agg[active] / max(1, len(idxs))
    df_imp = pd.DataFrame({"feature": names, "importance": values.numpy()})
...
def main(args):
    ...
    # KFold split (fix: don't rely on graph_data.x)
    N = labels.shape[0]
    ...
    trained_model = None  # keep last trained model for IG

    for fold, (train_idx, val_idx) in enumerate(split_iter):
        print(f'\nFold {fold + 1}/{args.k}')
        model = get_model(
            args.arch, args.modality, input_dim_text, input_dim_mel, num_classes,
        ...
        for epoch in tqdm(range(args.epochs), desc=f"Training Fold {fold+1}", leave=False):
            ...
            if getattr(model, "is_mlp", False):
                if args.modality == 'both':
                    x = torch.cat([graph_data.text, graph_data.mel], dim=1).to(args.device)
                elif args.modality == 'text':
                    x = graph_data.text.to(args.device)
    ...
    # --------- Explainability (IG) for MLP ----------
    if args.explain:
        if not getattr(trained_model, "is_mlp", False):
            print("[WARN] --explain set, but last trained model is not MLP. Skipping IG.")
        else:
            if args.modality == "text":
                x_all = graph_data.text
                feature_names = text_feature_names
                text_dim = len(text_feature_names)
            elif args.modality == "mel":
                x_all = graph_data.mel
                feature_names = mel_feature_names
                text_dim = 0
            elif args.modality == "both":
                x_all = torch.cat([graph_data.text, graph_data.mel], dim=1)
                feature_names = text_feature_names + mel_feature_names
                text_dim = len(text_feature_names)
            else:
                raise ValueError("Invalid modality")

            branch = args.explain_branch
            if branch == "both" and args.modality != "both":
                print("[WARN] --explain_branch both requested but modality is not 'both'; defaulting to active features.")
                branch = "text" if args.modality == "text" else "mel"

            print(f"[INFO] Computing Integrated Gradients for MLP (branch={branch}) ...")
            df_imp = compute_global_feature_importance_mlp(
                trained_model, x_all, feature_names, branch=branch,
                text_dim=text_dim, steps=args.ig_steps,
                sample_count=args.explain_samples, device=args.device
            )
            fi_csv = results_dir / f"feature_importance_{slug}_MLP_{branch}.csv"
            df_imp.to_csv(fi_csv, index=False)
            print(f"[OK] Wrote feature importances → {fi_csv}")

            fi_png = results_dir / f"feature_importance_{slug}_MLP_{branch}.png"
            title = f"Global Feature Importance (IG) — MLP {args.modality} ({args.classification}) [{branch}]"
...
if __name__ == '__main__':
    ...
    # MLP options (width/reg); layer count is auto-set by modality to ensure fair comparison-by-layers
    parser.add_argument('--mlp_hidden_dim', type=int, default=512)
    parser.add_argument('--mlp_dropout', type=float, default=0.0)

    # Explainability controls
    parser.add_argument('--explain', action='store_true', help="Compute feature importances (IG) and save barplot for MLP")
    parser.add_argument('--explain_branch', type=str, choices=['text','mel','both'], default='text',
                        help="Which input subset to attribute (for modality='both', you may choose 'both')")
    parser.add_argument('--explain_topk', type=int, default=20, help="Top-K features to show in the barplot")
    parser.add_argument('--explain_samples', type=int, default=0, help="Number of samples for IG (0=all)")
    parser.add_argument('--ig_steps', type=int, default=64, help="Riemann steps for Integrated Gradients")
